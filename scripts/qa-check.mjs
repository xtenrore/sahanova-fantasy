...
const [css,app,index,manifestRaw,sw,worker,wrangler,admin]=await Promise.all(['public/styles.css','public/js/app.js','public/index.html','public/manifest.webmanifest','public/sw.js','src/worker.js','wrangler.jsonc','public/js/admin.js'].map(f=>readFile(f,'utf8')));
...
assert.ok(worker.includes('/api/public/bootstrap')&&worker.includes('/api/admin/overrides')&&worker.includes('PBKDF2'),'Cloudflare Worker API/auth markers missing');
assert.ok(wrangler.includes('sahanova-db')&&wrangler.includes('run_worker_first'),'Cloudflare D1/static-assets routing missing');
assert.ok(app.includes('/api/public/bootstrap')&&admin.includes('/api/admin/overrides'),'D1-backed client bootstrap/admin persistence missing');
console.log('QA static audit OK — i18n parity, responsive classes, fullscreen, PWA, fantasy interactions, and Cloudflare Worker/D1 integration validated.');
