...
const required=[
 ...
 'server.mjs','src/node-session.js','README.md','.env.example','.gitignore','.github/workflows/quality.yml','RAILWAY_DEPLOY.md'
];
...
for(const f of ['public/js/app.js','public/js/data.js','public/js/domain.js','public/js/state.js','public/js/i18n.js','public/js/icons.js','public/js/admin.js','server.mjs','src/node-session.js']){
 const r=spawnSync(process.execPath,['--check',f],{encoding:'utf8'});if(r.status!==0){console.error(r.stderr);process.exit(r.status||1)}
}
console.log(`Build check OK — ${required.length} required artifacts and Railway Node server JavaScript syntax validated.`);
